...
console.log(students)
    const handleView = async (studentAssignmentId) => {
        try {
            ...
            if (error.response && error.response.data && error.response.data.message) {
                ...
                toast.error(language === 'En' ? 'Error viewing assignment' : 'خطأ في عرض التكليف');
            }
        }
    };
    ...
    const saveScore = async (studentAssignmentId) => {
        try {
            await assignmentsApi.editStudentAssignmentScore(studentAssignmentId, newScore);
            refreshData();
            setEditingScore(null);
        } catch (error) {
            toast.error(language === 'En' ? 'Error updating score' : 'خطأ في تحديث النتيجة');
        }
    };
    ...
    return (
        <div>
            ...
            <div>
                    ...
                    <table>
                        ...
                        {students.map((student, index) => (
                            <tr key={index}>
                                ...
                                <td>{student.submitted_file_name ? language === 'En' ? 'Submitted' : 'تم التسليم' : language === 'En' ? 'Not Submitted' : 'لم ينم التسليم'}</td>
                                <td>
                                    {editingScore === student.student_assignment_id ? (
                                        <input
                                            ...
                                            placeholder={language === 'En' ? 'Enter score' : 'أدخل الدرجة'}
                                        />
                                    ) : (
                                ...
                                <td>
                                    {!editingScore && student.student_file && (
                                        <button className="submit-edit-button" onClick={() => handleView(student.student_assignment_id)}>
                                    ...
                                    {editingScore === student.student_assignment_id ? (
                                        <>
                                            <button className="submit-edit-button"
                                                    onClick={() => saveScore(student.student_assignment_id)}>
                                        ...
                                        <button className="submit-edit-button"
                                                onClick={() => handleEditClick(student.student_assignment_id, student.score)}>
                                            {student.score ? language === 'En' ? 'Edit Score' : 'تعديل الدرجة' : language === 'En' ? 'Give Score' : 'إعطاء درجة'}
                                        </button>
                                    )}
                                </td>
                            </tr>
                ...
                {fileView && <div className={`StudentsAssignments_overlay-background ${isDarkMode ? 'dark' : ''}`} onClick={() => setFileView(null)}></div>}
                {fileView && (
                    <div className={`StudentsAssignments_overlay ${isDarkMode ? 'dark' : ''}`}>
